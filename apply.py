...
rep(
"function seed(){ return {lang:\"es\",name:\"\",startDate:today(),activeDay:0,routine:defaultRoutine(),logs:[],yogaLogs:[],journal:[],weights:[],photos:[],measures:[],cycle:[],settings:{weeklyGoal:3,cycleLen:28},machineSettings:{},programStartDate:today(),gems:0,frozenWeeks:[],prCount:0,medalsSeen:[],routineMig:true,onboarded:false,session:null}; }",
"function seed(){ return ensureRunningDB({lang:\"es\",name:\"\",startDate:today(),activeDay:0,routine:defaultRoutine(),logs:[],yogaLogs:[],journal:[],weights:[],photos:[],measures:[],cycle:[],settings:{weeklyGoal:3,cycleLen:28},machineSettings:{},programStartDate:today(),gems:0,frozenWeeks:[],prCount:0,medalsSeen:[],routineMig:true,onboarded:false,session:null}); }",
"seed",
)
rep(
"if(!Array.isArray(DB.yogaLogs))DB.yogaLogs=[]; save(); applyStatic(); closeModal(); go(\"inicio\"); alert(t('imported'));",
"if(!Array.isArray(DB.yogaLogs))DB.yogaLogs=[]; ensureRunningDB(DB); save(); applyStatic(); closeModal(); go(\"inicio\"); alert(t('imported'));",
"import",
)
rep(
...
""" if(v===\"entrenar\"){ if(trainPane===\"gym\")showTrainGym(); else if(trainPane===\"run\")showTrainRun(); else showTrainHub(); }""",
"go",
)
